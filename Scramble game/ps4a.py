...
def loadWords():
    ...
    # inFile: file
    inFile = open("C:\\Users\\NIMMY\\programmes\\python\\word game\\words.txt", 'r')
...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList:
        wp[0] = True
    for x in word:
        if x in hand.keys() and hand[x] >= word.count(x):
            count += 1
    if count == len(word):
        wp[1] = True
    return all(wp)
